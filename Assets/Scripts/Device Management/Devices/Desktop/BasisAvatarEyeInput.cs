...
public class BasisAvatarEyeInput : MonoBehaviour
{
    ...
    public void HandleMouseRotation(Vector2 lookVector)
    {
        ...
        {
            adjustedHeadPosition.y -= Eye.RestingLocalSpace.Position.y * crouchPercentage;
        }
    ...
    public void CalculateAdjustment()
    {
        ...
        {
            // Positive rotation
            adjustment = Mathf.Abs(rotationY) * (headDownwardForce / Eye.RestingLocalSpace.Position.y);
        }
        else
        {
            // Negative rotation
            adjustment = Mathf.Abs(rotationY) * (headUpwardForce / Eye.RestingLocalSpace.Position.y);
        }
    }
}