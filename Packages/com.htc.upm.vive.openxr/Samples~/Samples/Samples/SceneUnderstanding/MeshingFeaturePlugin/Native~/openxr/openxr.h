...
typedef enum XrStructureType {
    ...
    XR_TYPE_SYSTEM_EXPRESSION_PROPERTIES_HTC = 1000104000,
    XR_TYPE_EXPRESSION_CREATE_INFO_HTC = 1000104001,
    XR_TYPE_EXPRESSION_DATA_HTC = 1000104002,
...
typedef enum XrObjectType {
    ...
    XR_OBJECT_TYPE_EXPRESSION_HTC = 1000104000,
...
#define XR_HTC_facial_expression 1
XR_DEFINE_HANDLE(XrExpressionHTC)
#define XR_HTC_facial_expression_SPEC_VERSION 1
#define XR_HTC_FACIAL_EXPRESSION_EXTENSION_NAME "XR_HTC_facial_expression"

typedef enum XrEyeShapeHTC {
    XR_EYE_SHAPE_NONE_HTC = -1,
    XR_EYE_SHAPE_EYE_LEFT_BLINK_HTC = 0,
    XR_EYE_SHAPE_EYE_LEFT_WIDE_HTC = 1,
    XR_EYE_SHAPE_EYE_RIGHT_BLINK_HTC = 2,
    XR_EYE_SHAPE_EYE_RIGHT_WIDE_HTC = 3,
    XR_EYE_SHAPE_EYE_LEFT_SQUEEZE_HTC = 4,
    XR_EYE_SHAPE_EYE_RIGHT_SQUEEZE_HTC = 5,
    XR_EYE_SHAPE_MAX_ENUM_HTC = 0x7FFFFFFF
} XrEyeShapeHTC;

typedef enum XrLipShapeHTC {
    ...
    XR_LIP_SHAPE_MAX_ENUM_HTC = 0x7FFFFFFF
} XrLipShapeHTC;

typedef enum XrExpressionTypeHTC {
    XR_EXPRESSION_TYPE_EYE_HTC = 1,
    XR_EXPRESSION_TYPE_LIP_HTC = 2,
    XR_EXPRESSION_TYPE_MAX_ENUM_HTC = 0x7FFFFFFF
} XrExpressionTypeHTC;
// XrSystemExpressionPropertiesHTC extends XrSystemProperties
typedef struct XrSystemExpressionPropertiesHTC {
    XrStructureType       type;
    void* XR_MAY_ALIAS    next;
    XrBool32              supportEyeExpression;
    XrBool32              supportLipExpression;
    uint32_t              maxBlendShapesCount;
} XrSystemExpressionPropertiesHTC;

typedef struct XrExpressionDataHTC {
    XrStructureType       type;
    const void* XR_MAY_ALIAS    next;
    XrTime                sampleTime;
    uint32_t                    blendShapesCapacityInput;
    uint32_t                    blendShapesCountOutput;
    float*                      blendShapeWeightings;
} XrExpressionDataHTC;

typedef struct XrExpressionCreateInfoHTC {
    XrStructureType             type;
    const void* XR_MAY_ALIAS    next;
    XrExpressionTypeHTC         expressionType;
} XrExpressionCreateInfoHTC;

typedef XrResult (XRAPI_PTR *PFN_xrCreateExpressionHTC)(XrInstance instance, const XrExpressionCreateInfoHTC* createInfo, XrExpressionHTC* expression);
typedef XrResult (XRAPI_PTR *PFN_xrDestroyExpressionHTC)(XrExpressionHTC expression);
typedef XrResult (XRAPI_PTR *PFN_xrGetExpressionDataHTC)(XrExpressionHTC expression, XrExpressionDataHTC* expressionData);

#ifndef XR_NO_PROTOTYPES
#ifdef XR_EXTENSION_PROTOTYPES
XRAPI_ATTR XrResult XRAPI_CALL xrCreateExpressionHTC(
    XrInstance                                  instance,
    const XrExpressionCreateInfoHTC*            createInfo,
    XrExpressionHTC*                            expression);

XRAPI_ATTR XrResult XRAPI_CALL xrDestroyExpressionHTC(
    XrExpressionHTC                             expression);

XRAPI_ATTR XrResult XRAPI_CALL xrGetExpressionDataHTC(
    XrExpressionHTC                             expression,
    XrExpressionDataHTC*                        expressionData);
#endif /* XR_EXTENSION_PROTOTYPES */
#endif /* !XR_NO_PROTOTYPES */
